...
export const getPathsBySection = async (sectionHandle, limit) => {
	...
	const sectionHandleToUse = Array.isArray(sectionHandle)
		? sectionHandle.reduce((previousValue, currentValue, currentIndex) => {
				if (currentIndex == 1) return `["${previousValue}","${currentValue}"`;

				return `${previousValue}, "${currentValue}"${
					currentIndex == sectionHandle.length - 1 && "]"
				}`;
		  })
		: `"${sectionHandle}"`;
	const paths = await fetchAPI(`
    query paths{
      entries(section: ${sectionHandleToUse}, limit: ${limit ? limit : 1000}){
        slug
      }
    }
  `);
