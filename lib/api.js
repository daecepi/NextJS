...
import { basicEntryFieldsToGet } from "./graphql/fieldGroups/baseFIeldGroups";
...
export const getBlogResultPages = (query) => {
	const definingFilterString = query ? `(section: "${query}")` : "";
	const data = fetchAPI(`
  query BlogResults{
    entries${definingFilterString}{
...
export const getEventListingEntries = (query) => {
	const data = fetchAPI(`
  query BlogResults{
    entries(section: "eventListing"){
      ${basicEntryFieldsToGet}
    }
  }
  `);
