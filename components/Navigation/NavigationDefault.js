...
const NavigationDefault = () => {
  ...
  return (
    <nav className="fixed top-0 w-full hidden h-24 lg:flex justify-center z-30">
          ...
          <ul className="flex items-center h-24">
            {menu.map((item) => {
              ...
              return (
                <li
                  className={`${
                    active == item[itemKey].tag
                      ? "border-b-4 border-radicalRed-500 pt-1"
                      : ""
                  } flex items-center h-full mr-4 hover:border-b-4 hover:border-radicalRed-500 hover:pt-1`}
                >
                  <div
                    className="cursor-pointer "
                    onClick={() => toggleMenu(item[itemKey].tag)}
                  >
                    {item[itemKey].dropdown ? (
                      <p>{item[itemKey].name}</p>
                    ) : (
                  ...
                  {item[itemKey].dropdown ? (
                    // condition if dropdown
                    <ul
                      className={` ${
                        active == item[itemKey].tag ? "block" : "hidden"
                      }  pt-10 pb-10 flex absolute top-full h-60 w-full max-w-7xl px-20 left-1/2 transform -translate-x-1/2 bg-white`}
                    >
                      <ul className="w-1/3">
                        {item[itemKey].dropdown
                          ? item[itemKey].dropdown.map(
                              (dropdownItem, index) => {
                                if (index <= 2)
                                  return (
                                    <li>
