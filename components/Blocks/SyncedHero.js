...
<script>
  ...
  `;
	...
	for (let i = 0; i < successMessageSpecializations.length; i++) {
		success.push({
			variationLogic: i.variationLogic.value,
			variationLogicComparison: i.variationLogicComparison,
			variationLogicValue: i.variationLogicValue,
			vSuccessMessage: i.vSuccessMessage,
		});
	}
	const contentRatios = {
		normal: {
	...
	let ratioSelectedOnBackend = copyRatioVariation
		? copyRatioVariation.value
		: undefined;

	const specificBackground = backgroundColor?.length
		? backgroundColor[0]
		: "hot-pink";
	...
	const hardCodedVideo = () => {
		return (
			...
			>
				<script
					src="https://fast.wistia.com/embed/medias/pmc7iaf2o2.jsonp"
					async
				></script>
				<script
				...
				>
					...
					>
						<div
							className="wistia_embed wistia_async_pmc7iaf2o2 videoFoam=true"
						...
						>
							...
							>
								<img
									src="https://fast.wistia.com/embed/medias/pmc7iaf2o2/swatch"
									style={{
										filter: "blur(5px)",
										height: "100%",
										objectFit: "contain",
			...
			case "halfFullImageHero":
				return (
					<>
						<section
							id="defaultVariation"
							className={`c-hero--split landing-hero ${specificBackground} no-transition synced-variation`}
						>
									...
									<div id="default-asset-container" className="c-image">
										{defaultImage[0].optimizedLandingImages?.length > 0 ? (
											<Image
												...
												width={
													defaultImage[0].optimizedLandingImages
														.originalImageWidth
												}
												height={
													defaultImage[0].optimizedLandingImages
											...
											<img
												...
												alt={
													defaultImage[0].altText?.length
														? defaultImage[0].altText
														: defaultImage[0].title
												}
											/>
										)}
									</div>
							...
							<div className="container">
								<div className="row row--no-reverse">
									<div
										className={`${
											ratioSelectedOnBackend &&
											contentRatios[ratioSelectedOnBackend]
												? contentRatios[ratioSelectedOnBackend].copy
												: contentRatios["normal"].copy
										} c-valign--middle synced-hero-copy`}
									>
										<div className="hero-text">
											{defaultEyebrow?.length ? (
												<pre
													className={`c-eyebrow c-eyebrow--${
														eyebrowSettings[0].eyebrowColor &&
														eyebrowSettings[0].eyebrowColor[0]
											...
											{defaultCopy ? (
												<div
													dangerouslySetInnerHTML={{ __html: defaultCopy }}
												></div>
											) : (
												<>
													<h2>The real estate CRM that’s move-in ready.</h2>
													<p>
														Copper is the CRM of choice for real estate agents
														and brokers.
													</p>
												</>
											)}
										...
										<div
											id="initial-control"
											className="c-hero__buttons c-finder"
										>
											<form id="finder-form">
												<input
													id="emailFinder"
													className="finder-input"
													onkeyup="formUpdater(this.value)"
													type="email"
													placeholder="Add your email"
													required
												/>
												<button
													id="email-finder-submit"
													className="c-button c-button--white"
													type="submit"
												>
													{differentTryFreeCtaCopy?.length
											...
											<div id="c-thank-you-message" className="invisible-start">
												{marketoThankYouMessage?.length ? (
													...
													<>
														<h3>
															Thank{" "}
															<span className="hot-pink t-hot-pink">you</span>.
														</h3>
														<p className="p-sm">
							...
							<div className="container">
								<div className="row row--no-reverse">
									...
									{displayVideo ? (
										...
										>
											<div className="c-video ">{hardCodedVideo()}</div>
										</div>
									)}
										...
										<div
											...
											className={`${
												...
												contentRatios[ratioSelectedOnBackend]
													? contentRatios[ratioSelectedOnBackend].copy
													: contentRatios["normal"].copy
											} c-valign--middle c-image invert-sm-spacing c-valign--middle"`}
				...
				>
					<div className="container">
						<div className="row row--no-reverse">
									...
									>
										{successMessage?.length ? (
											...
											<>
												<h2>
													Thanks! <br />
													Watch and see real estate in action.{" "}
												</h2>
												<p>TBD copy here</p>
											</>
										)}
							...
							{displayVideoOnSuccess ? (
								<div
									id="final-asset-container"
									className="col-md-6 offset-md-1 c-valign--middle"
								>
									<div className="c-video ">
										{successVideoEmbedCode?.length ? (
											<div
												dangerouslySetInnerHTML={{
													__html: successVideoEmbedCode,
												}}
											></div>
										) : (
											<>{hardCodedVideo()}</>
								...
								<div
									id="final-asset-container"
									className="col-md-6 offset-md-1 c-valign--middle c-image invert-sm-spacing c-valign--middle"
									style={{ position: "relative", left: "0px" }}
								>
									<img
										style={{ objectFit: "initial", height: "auto" }}
