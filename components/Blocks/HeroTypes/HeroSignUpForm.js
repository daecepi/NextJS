	...
	return (
		<section className="hero-signup-form -mt-28 pt-36 pb-10 relative w-full min-h-screen flex align-middle">
			<div className="hero-signup-form-container max-w-7xl m-auto flex flex-wrap z-10">
				<div
					className={`hero-signup-form__form w-full md:w-1/2 ${
						props.signupSide == "left" ? " order-1" : " order-2"
					}`}
				>
					<form
				...
				<div
					className={`hero-signup-form__text-container w-full md:w-1/2 flex flex-col z-10 justify-center ${
						props.signupSide == "left" ? " order-2" : " order-1"
					}`}
				>
					<div
			...
			<div
				className={`hero-signup-form__background absolute top-0 w-1/2 h-full z-0 bg-${
					props.containerBg[0].slug
				}-default ${props.signupSide == "left" ? " right-0" : " left-0"}`}
			></div>
		</section>
	);
};
